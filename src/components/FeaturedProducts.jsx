...
const FeaturedProducts = ({ lang = 'ar' }) => {
    ...
    const t = {
        ar: {
            ...
            subtitle: 'اخترنا لك الأفضل من مجموعتنا المتنوعة من الأشجار والنباتات',
            new: 'جديد!',
        ...
        fr: {
            ...
            subtitle: 'Nous avons sélectionné pour vous le meilleur de notre gamme variée d\'arbres et de plantes',
            new: 'Nouveau!',
        ...
        <section className="py-20 bg-gradient-to-b from-white to-gray-50">
            <div className="container mx-auto px-4">
                <div className="text-center mb-14">
                    <h2 className="text-4xl md:text-5xl font-bold text-gray-800 mb-4">{text.title}</h2>
                    <div className="w-24 h-1 bg-gradient-to-r from-green-500 to-green-600 mx-auto mb-4"></div>
                    <p className="text-xl text-gray-600 max-w-2xl mx-auto">{text.subtitle}</p>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    {featured.map((product) => (
                        <Link to={isFrench ? `/fr/products/${product.id}` : `/products/${product.id}`} key={product.id} className="group bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-3 border border-gray-100 relative">
                ...
                <div className="text-center mt-12">
                    <Link to={isFrench ? "/fr/products" : "/products"} className="inline-block border-2 border-green-600 text-green-600 font-bold py-3 px-8 rounded-full hover:bg-green-600 hover:text-white transition">
                        {text.viewAll}
                    </Link>
                </div>
            </div>
